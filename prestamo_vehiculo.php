...
if ($permisos->{'prestamo_vehiculo'} == "on"  || isset($_POST['location'])) {
  include_once("header/menu.php");
  include "php/prestamocontroller.php";
  include "modal/modal_crear_prestamo_vehiculo.php";
?>
...
<li class="breadcrumb-item active">Historial de Préstamos de Vehículos</li>
</ol>
...
<div class="card-header font-weight-bold text-<?php echo $font; ?> bg-<?php echo $barcolor; ?>" style="margin-top: 0px !important;">
 <p> <i class="fas fa-fw fa-car"></i> Histórico de Préstamos de Vehículos</p>
	
  <a href="backup_datos/backup_prestamo.php?negocio=<?php echo $negocio; ?>&tipo_negocio=Prestamo Vehiculo" style="float: right;color:#fff;margin-left:10px;"class="btn btn-success btn-mdax">Exportar</a>
	
  <?php if($negocio == 0 || ($cant_prestamo > $cant_prestamor_actual)){ ?>
  <button style="float: right;" class="btn btn-light btn-mdax"  onclick="new_prestamo('<?php echo $user_id; ?>')" data-toggle="modal" data-target="#modal_crear_prestamo_vehiculo">Nuevo</button>
  <?php } ?>
...
<?php
    ...
    $rows = $du->read_all_prestamo("Prestamo Vehiculo",$negocio);
    if ($rows->num_rows >= 1) {
...
<script type="text/javascript" src="js/js_prestamo_vehiculo.js"></script>
<?php 
