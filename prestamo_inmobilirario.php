...
if ($permisos->{'prestamo_inmobilirario'} == "on"  || isset($_POST['location'])) {
  include_once("header/menu.php");
  include "php/prestamocontroller.php";
  include "modal/modal_prestamo_inmobiliario.php";
?>
...
<li class="breadcrumb-item active">Historial de Préstamos Inmobiliarios</li>
</ol>
<div class="card mb-3" id="table_prt_inmobiliario">
<div class="card-header font-weight-bold text-<?php echo $font; ?> bg-<?php echo $barcolor; ?>" style="margin-top: 0px !important;">
  <p> <i class="fas fa-fw fa-building"></i> Histórico de Préstamos Inmobiliarios</p>
	
  <a href="backup_datos/backup_prestamo.php?negocio=<?php echo $negocio; ?>&tipo_negocio=Prestamo Inmobiliario" style="float: right;color:#fff;margin-left:10px;"class="btn btn-success btn-mdax">Exportar</a>

  <?php if($negocio == 0 || ($cant_prestamo > $cant_prestamor_actual)){ ?>
  <button style="float: right;" class="btn btn-light btn-mdax"  onclick="new_prestamo('<?php echo $user_id; ?>')" data-toggle="modal" data-target="#modal_crear_prestamo_inmobiliario">Nuevo</button>
  <?php } ?>
...
<?php
    ...
    $rows = $du->read_all_prestamo("Prestamo Inmobiliario",$negocio);
    if ($rows->num_rows >= 1) {
...
?>  
    <tr data-href="detalle_prestamo.php?tmp=<?php echo base64_encode($row->id_prestamo); ?>&tp=<?php echo base64_encode($row->tipo_prestamo); ?>">
      ...
      <td><?php print($row->proximo_pago) ?></td>
    </tr>
<?php } ?>
<?php
     }else{
...
<script type="text/javascript" src="js/js_prestamo_inmobiliiario.js"></script>
<?php 
