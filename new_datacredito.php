...
$query = "SELECT CONCAT(clt.nombre,' ',clt.apellido) as cliente,clt.cedula,clt.direccion,clt.telefono,clt.celular,prt.fecha_creado, prt.capital_inicial,amrt.monto_pagar as balance_al_dia,htp.fecha as ultimo_pago,prt.fecha,prt.capital_pendiente,prt.estado,prt.ciclo_pago,prt.cuota FROM clientes AS clt 
...
INNER JOIN historial_pagos AS htp ON prt.id_prestamo = htp.id_prestamo 
WHERE clt.negocio = 0 and prt.estado != 'Cancelado'
GROUP BY prt.id_prestamo ORDER BY prt.id_prestamo ASC,htp.fecha DESC";
$run = $con->query($query);
