...
<li class="breadcrumb-item active">Graficas de Estadisticas</li>
</ol>
<?php 
	...
	$query = "SELECT buscar_mes(MONTH(htp.fecha)) AS mes,
				...
				INNER JOIN clientes AS clt ON prt.id_cliente = clt.id_cliente 
				WHERE htp.estado = 'Pagada' and	clt.negocio = $negocio
          ...
          <div class="col-xl-3 col-sm-6 mb-3">
            <div class="card text-white bg-success o-hidden h-100">
              <div class="card-body">
                ...
                <div class="mr-5">Prestamos Activos: <?php echo $total_activos; ?></div>
              </div>
          ...
          <div class="col-xl-3 col-sm-6 mb-3">
            <div class="card text-black bg-warning o-hidden h-100">
              <div class="card-body">
                ...
                <div class="mr-5">Prestamos Atrasados: <?php echo ($total_atrasado); ?></div>
              </div>
          ...
          <div class="col-xl-3 col-sm-6 mb-3">
            <div class="card text-white bg-info o-hidden h-100">
              <div class="card-body">
                ...
                <div class="mr-5">Prestamos en Legal: <?php echo $total_asunt_legal; ?></div>
              </div>
              <!--<a class="card-footer text-white clearfix small z-1" href="#">
                <span class="float-left">Ver Detalles</span>
                <span class="float-right">
          ...
          <div class="col-xl-3 col-sm-6 mb-3">
            <div class="card text-white bg-danger o-hidden h-100">
              <div class="card-body">
                ...
                <div class="mr-5">Prestamos Cancelados: <?php echo $total_cancelado; ?></div>
              </div>
              <!--<a class="card-footer text-white clearfix small z-1" href="#">
                <span class="float-left">Ver Detalles</span>
                <span class="float-right">
	   ...
	   <tbody  class="hoverclass" id="table_general">
		 <?php
		   $query = "SELECT prt.id_prestamo,prt.tipo_prestamo, prt.cuota, prt.capital_pendiente, cantidad_faltas(prt.ciclo_pago,prt.proximo_pago) AS faltas,
			(CASE
			WHEN proximo_pago < NOW() and cantidad_faltas(prt.ciclo_pago,prt.proximo_pago) >= 1 THEN  'Atrasado'
			WHEN  proximo_pago < NOW() and cantidad_faltas(prt.ciclo_pago,prt.proximo_pago) = 0 THEN 'Mora'
			WHEN  cantidad_faltas(prt.ciclo_pago,prt.proximo_pago) = 0 THEN 'Al dia'
			END)  as esta,
			...
			YEAR(prt.proximo_pago) <= YEAR(NOW())) and
		 ...
		 <?php } ?>
           <td>
			   	<div class="form-row">
					<div class="form-group col">
						<label style="font-size:20px;font-weight:bold;"><?php echo $row->cliente; ?></label>
					</div>
					<div class="form-group col" align="right">
		   				<span>Cuota: <?php echo "RD$ ".number_format($row->monto_pagar,2); ?></span>
					</div>
				</div>
			   	<div class="form-row">
					<div class="form-group col">
						<label>Capt. Actual: <?php echo "RD$ ".number_format($row->capital_pendiente,2); ?></label>
						<br>
						<label>Cuot. Paga: <?php echo ($row->cuota-$row->no_cuota)."/".$row->cuota; ?></label>
					</div>
					<div class="form-group col" align="right">
					<?php if($row->esta == "Al dia"){ ?>
		   				<label>Pendiente</label>
					<?php }elseif($row->esta == "Atrasado"){ ?>
		   				<label><?php if($row->faltas > $row->no_cuota){ echo $row->no_cuota; }else{ echo $row->faltas;} ?> Cuotas Pendientes</label>
					<?php }elseif($row->esta == "Mora"){ ?>
		   				<label>Mora</label>
					<?php } ?><br>
...
<div class="card-footer" style="position: absolute;bottom: 0;width: 97.5%;" align="right">
...
<div class="tab-contents tab-pane fade" id="container_al_dia"  style="overflow:auto;">
	...
	<table class="table table-hover table-bordered"  width="100%" cellspacing="0">
	   <tbody  class="hoverclass" id="table_general_pendiente">
		 <?php
...
<div class="card-footer" style="position: absolute;bottom: 0;width: 97.5%;">
...
<div class="tab-contents tab-pane fade" id="container_mora"  style="overflow:auto;">
	...
	<table class="table table-hover table-bordered"  width="100%" cellspacing="0">
	   <tbody class="hoverclass" id="table_general_mora">
		 <?php
		   $query = "SELECT prt.id_prestamo,prt.tipo_prestamo,amrt.no_cuota, prt.cuota, prt.capital_pendiente, cantidad_faltas(prt.ciclo_pago,prt.proximo_pago) AS faltas,
			(CASE
				WHEN proximo_pago < NOW() and cantidad_faltas(prt.ciclo_pago,prt.proximo_pago) >= 1 THEN  'Atrasado'
           ...
           <td>
			   	...
			   	<div class="form-row">
					...
					<?php if($row->esta == "Al dia"){ ?>
		   				<label>Pendiente</label>
					<?php }elseif($row->esta == "Atrasado"){ ?>
		   				<label><?php if($row->faltas > $row->no_cuota){ echo 1; }
															else{ echo $row->faltas;} ?> Cuotas Pendientes</label>
					<?php }elseif($row->esta == "Mora"){ ?>
		   				<label>Mora</label>
					<?php } ?><br>
					<label><?php echo $row->proximo_pago; ?></label>
...
<div class="tab-contents tab-pane fade" id="container_atrasado"  style="overflow:auto;">
	...
	<table class="table table-hover table-bordered"  width="100%" cellspacing="0">
	   <tbody class="hoverclass"  id="table_general_atrasado">
		 <?php
		   $query = "SELECT prt.id_prestamo,prt.tipo_prestamo,amrt.no_cuota, prt.cuota, prt.capital_pendiente, cantidad_faltas(prt.ciclo_pago,prt.proximo_pago) AS faltas,
			(CASE
				WHEN proximo_pago < NOW() and cantidad_faltas(prt.ciclo_pago,prt.proximo_pago) >= 1 THEN  'Atrasado'
         ...
         <tr data-href="detalle_prestamo.php?tmp=<?php echo base64_encode($row_atraso->id_prestamo); ?>&tp=<?php echo base64_encode($row_atraso->tipo_prestamo); ?>" style="cursor:pointer;background-color: #f2dede !important;">
           <td>
			   	<div class="form-row">
					<div class="form-group col">
						<label style="font-size:20px;font-weight:bold;"><?php echo $row_atraso->cliente; ?></label>
					</div>
					<div class="form-group col" align="right">
		   				<span>Cuota: <?php echo "RD$ ".number_format($row_atraso->monto_pagar,2); ?></span>
					</div>
				</div>
			   	<div class="form-row">
					<div class="form-group col">
						<label>Capt. Actual: <?php echo "RD$ ".number_format($row_atraso->capital_pendiente,2); ?></label>
						<br>
						<label>Cuot. Paga: <?php echo ($row_atraso->cuota-$row_atraso->no_cuota)."/".$row_atraso->cuota; ?></label>
					</div>
					<div class="form-group col" align="right">
					<?php if($row->esta == "Al dia"){ ?>
		   				<label>Pendiente</label>
					<?php }elseif($row_atraso->esta == "Atrasado"){ ?>
		   				<label><?php if($row_atraso->faltas > $row_atraso->no_cuota){ echo $row_atraso->no_cuota; }else{ echo $row_atraso->faltas;} ?> Cuotas Pendientes</label>
...
<div class="card-footer" style="position: absolute;bottom: 0;width: 97.5%;" align="">
...
<div class="tab-contents tab-pane fade" id="container_legal"  style="overflow:auto;">
	...
	<table class="table table-hover table-bordered"  width="100%" cellspacing="0">
	   <tbody class="hoverclass" id="table_general_legal">
		 <?php
		   $query = "SELECT prt.id_prestamo,prt.tipo_prestamo,amrt.no_cuota, prt.cuota, prt.capital_pendiente, cantidad_faltas(prt.ciclo_pago,prt.proximo_pago) AS faltas,
			(CASE
			WHEN proximo_pago < NOW() and cantidad_faltas(prt.ciclo_pago,prt.proximo_pago) >= 1 THEN  'Atrasado'
...
function initMap() {
  ...
  function updateMaps() {
    ...
    var url = 'https://puntocredito.net/keys/imarkerFirst.php?t='+timestamp;
...
Highcharts.chart('container_ingresos', {
    ...
    title: {
        text: 'Informacion General de Ingresos'
    },
    subtitle: {
...
Highcharts.chart('container_cobrados', {
    ...
    title: {
        text: 'Informacion General de Cobrados'
    },
    subtitle: {
        text: 'Click para Ver Detalles'
    },
